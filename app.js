...
var CFG={lives:3,tries:5,candidates:'on',wordbank:'full',radius:20,theme:'light',scale:1.3,blur:24,llmApiKey:'',llmModel:'GLM-5.1',llmHost:'https://cmkey.cn/v1',llmCors:false};
var S={},typed='',tickId=null,defCache={};
var $=function(id){return document.getElementById(id);};

try{var sv=JSON.parse(localStorage.getItem('ll_cfg'));if(sv)Object.assign(CFG,sv);}catch(e){}
applyCfg();
...
document.querySelectorAll('[data-screen]').forEach(function(b){b.addEventListener('click',function(){
  if(b.classList.contains('menu-btn')&&!Dict.isReady())return;
  showScreen(b.getAttribute('data-screen'));
});});
document.querySelectorAll('.diff-btn').forEach(function(b){b.addEventListener('click',function(){
...
function applyCfg(){
  ...
  // Liquid Glass engine
  if(CFG.theme==='liquid'){
    setTimeout(function(){ LiquidEngine.activate(); },100);
  } else {
...
function startGame(mode,botId,timerSec){
  ...
  S={mode:actualMode,isLearn:isLearn,botId:botId||null,timerSec:timerSec,timerTicks:timerSec*10,timer:timerSec*10,
...
function endGame(){
  ...
  saveHist({mode:S.mode,bot:bn,score:S.mode==='endless'?S.usedWords.length:S.score,rounds:S.round,words:S.usedWords.length,date:Date.now(),wordList:S.usedWords.slice(0,200)});
  showScreen('screen-gameover');
}

// Side defs (learn, no popup)
function addSideDef(word){Dict.fetchDefinition(word).then(function(d){S.learnedDefs.push({word:word,pos:d.pos,def:d.def});var el=document.createElement('div');el.className='def-entry';el.innerHTML='<div class="def-entry-word">'+word+'</div>'+(d.pos?'<div class="def-entry-pos">'+d.pos+'</div>':'')+
...
function updateCands(){
  ...
  if(CFG.candidates==='on')cands.slice(0,50).forEach(function(w){var d=document.createElement('div');d.className='cand-word';d.textContent=w;l.appendChild(d);});
}
...
function addChain(word,pts,type){
  var div=document.createElement('div');div.className='chain-word '+type;
  if(type==='skip')div.innerHTML='<span class="cw-text">'+word+'</span>';
  else{var hl=S.prefix,rest=word.slice(hl.length),cls=type==='bot'?'cw-bot-hl':'cw-highlight';
    div.innerHTML='<span class="cw-text"><span class="'+cls+'">'+hl+'</span>'+rest+'</span><span class="cw-score">+'+pts+'</span>';}
  $('chain-words').appendChild(div);$('chain-area').scrollTop=$('chain-area').scrollHeight;
}
