...
var Dict = (function () {
    ...
    function computeEndings() {
        ...
        for (var i = 0; i < alphabet.length; i++) {
            var char = alphabet[i];
            var count = byPrefix[char] ? byPrefix[char].length : 0;
            counts.push({ char: char, count: count });
        }
        counts.sort(function(a, b) { return b.count - a.count; });
        easyEndings = counts.slice(0, 10).map(function(item) { return item.char; });
        hardEndings = counts.slice(-6).map(function(item) { return item.char; });
        console.log("Dynamic Dictionary Difficulty Analysis:");
        console.log("Easy Whitelist Endings (Top 10):", easyEndings.join(', '));
        console.log("Hard Blacklist Endings (Bottom 6):", hardEndings.join(', '));
    }
    ...
    async function loadCommonWords() {
        ...
        for (var url of urls) {
            try {
                var r = await fetchWithTimeout(url, 1500);
                if (r.ok) {
                    ...
                    if (count > 100) {
                        console.log("Loaded " + commonWords.size + " common words from " + url);
                        loaded = true;
                        break;
                    }
                }
    ...
    async function load(statusEl, bank) {
        ...
        bank = bank || 'sowpods';
        var urls = bank === 'sowpods'
            ? ['https://raw.githubusercontent.com/pbevin/lexicon/master/sowpods.txt']
            : bank === 'dwyl'
        ...
        if (bank === 'sowpods' && typeof SOWPODS_WORDS !== 'undefined' && SOWPODS_WORDS) {
            var list = SOWPODS_WORDS.match(/[a-z]{2,}/gi);
            if (list) {
                for (var i = 0; i < list.length; i++) {
        ...
        } else if (bank === 'dwyl' && typeof WORDS_ALPHA !== 'undefined' && WORDS_ALPHA) {
    ...
    return { load: load, isValid: isValid, isReady: isReady, getSize: getSize, findWords: findWords, countWords: countWords, fetchDefinition: fetchDefinition, injectDef: injectDef, words: words, byPrefix: byPrefix, getEasyEndings: function(){return easyEndings;}, getHardEndings: function(){return hardEndings;}, getCommonWords: function(){return commonWords;} };
})();
